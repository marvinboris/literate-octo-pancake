...
const appBlock = ({ children, animation = "fade-up", img, title = '', subtitle = '', link = '/', color = '', popular }) =>
    <div data-aos={animation} className="AppBlock p-3">
        <div className="bg-white rounded-4 hover">
            <div className={`shadow-sm bg-${popular ? `${color}-10` : "white"} position-relative overflow-hidden rounded-4 p-4`}>
                ...
                <div className="position-relative my-4">
                    <div className="position-absolute bg-border-50" style={{ height: 1, width: 'calc(100% + 3rem)', top: '50%', left: '-1.5rem', transform: 'translateY(-50%)' }} />

                    <div className={`py-2 px-4 bg-${color} shadow rounded-2 position-relative overflow-hidden text-white text-center`} style={{ height: 62, margin: popular && '0 -2rem' }}>
